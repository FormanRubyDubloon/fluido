  ...
  if (!loaded) {
    return (
      <div className="flex items-center justify-center h-64">
        <Icon name="progress_activity" size={40} className="animate-spin text-muted-foreground" />
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="flex items-center justify-between gap-4">
        ...
        <Select
          ...
          onValueChange={(v) => setSelectedDeck(v === "__all__" ? null : v)}
        >
      ...
      <section>
        <h3 className="text-sm font-medium text-muted-foreground mb-3">Card States</h3>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          ...
          <StatCard label="Suspended" value={String(cardCounts.suspended)} color="text-muted-foreground" />
          <StatCard label="Total" value={String(cardCounts.total)} />
        </div>
      </section>

      <section>
        <h3 className="text-sm font-medium text-muted-foreground mb-3">30-Day Retention</h3>
        <div className="text-3xl font-semibold">{retention30d > 0 ? `${retention30d}%` : "—"}</div>
        <p className="text-xs text-muted-foreground mt-1">Percentage of reviews rated Good or Easy</p>
      </section>

      {last30Days.length > 0 && (
        <section>
          <h3 className="text-sm font-medium text-muted-foreground mb-3">Reviews — Last 30 Days</h3>
      ...
      {forecast.some((f) => f.due > 0) && (
        <section>
          <h3 className="text-sm font-medium text-muted-foreground mb-3">Forecast — Next 30 Days</h3>
